...
class Recipe(Base):
    ...
    def return_ingredients_as_list(ingredients):
        if ingredients == "":
            ...
            return list(ingredients.split(', ').strip())
...
def create_recipe():
    ...
    recipe_entry.difficulty = recipe_entry.calc_difficulty(cooking_time, ingredients)
...
def edit_recipe():
    ...
    if session.query(Recipe).count() == 0:
        ...
        recipe_to_edit.difficulty = recipe_to_edit.calc_difficulty( recipe_to_edit.cooking_time, recipe_to_edit.ingredients)
